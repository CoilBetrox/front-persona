...
export default {
  ...
  methods: {
      async getUsuarios() {
          try {
              const response = await fetch('http://localhost:8081/personas');
      ...
      async putUsuario(usuario) {
          try {
              const response = await fetch(`http://localhost:8081/personas/${usuario.id}`, {
                  method: 'PUT',
                  body: JSON.stringify(usuario),
                  headers: { 'Content-type': 'application/json; charset=UTF-8' },
              });
      ...
      async deleteUsuario(usuario) {
          try {
              await fetch(`http://localhost:8081/personas/${usuario.id}`, {
                  method: "DELETE"
              });
